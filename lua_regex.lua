...
function Regex:sub_js(repl, str, count)
    if count and count <= 0 then return str, 0 end
    ...
    for match in self:finditer(str) do
        ...
        if isFunc then
			local params = {}
			local g = match:group(0)
			local gindex = 1
			while ( g ~= nil ) do
				table.insert(params, g)
				g = match:group(gindex)
				gindex = gindex + 1
			end
			
			local pos, _ = match:span(0)
			table.insert(params, pos-1)
			table.insert(params, str)
			
            r = repl(unpack(params))
            if r then
                r = tostring(r)
            else
                r = ""
...
function re.sub_js(regexp, repl, str, count, flags)
    return re.__getRegex(regexp, flags):sub_js(repl, str, count)
end
